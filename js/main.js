  ...
  switch (playerTurn % 2) {
    case 0:
      ...
      if (typeof origBoard[square.target.id] == "number") {
        if (removedPiece) {
          radius = parseFloat(
            radiusString.substring(
              radiusString.lastIndexOf(":") + 1,
              radiusString.lastIndexOf(";")
            )
          );
      ...
      // Action 2: Pick up a white piece.
        ...
        radius = parseFloat(
          radiusString.substring(radiusString.lastIndexOf(":") + 1, radiusString.lastIndexOf(";"))
        );
      ...
      // Action 3: Put a white piece that has been taken off the board and put it on another piece on the board.
        ...
        (origBoard[square.target.id] == "X" || origBoard[square.target.id] == "O") &&
        removedPiece
      ) {
        // Gobblet over own OR enemy
        radius = parseFloat(
          radiusString.substring(radiusString.lastIndexOf(":") + 1, radiusString.lastIndexOf(";"))
        );
      ...
      // Action 4: Put a white piece over a black piece IF black has three pieces in a row.
        ...
        checkThreeInARow(origBoard, playerO) &&
        origBoard[square.target.id] == "O" &&
        !removedPiece
      ) {
        radius = determineCircleRadius("WHITE");
    ...
    case 1:
      ...
      if (typeof origBoard[square.target.id] == "number") {
        ...
        checkThreeInARow(origBoard, playerX) &&
        origBoard[square.target.id] == "X" &&
        !removedPiece
      ) {
        radius = determineCircleRadius("BLACK");
        if (radius !== 0 && checkCellAvailablilty(radius, square.target.id)) {
...
function checkCellAvailablilty(circleRadius, id) {
  ...
  if (circleRadius - large > -0.1 && circleRadius - large < 0.1) {
    ...
    numberInCell = origBoardMemory[id][0];
    if (numberInCell == 0) numberInCell = origBoardMemory[id][1];
    return numberInCell == 0;
  } else if (circleRadius === small) {
    numberInCell = origBoardMemory[id][0];
    if (numberInCell == 0) numberInCell = origBoardMemory[id][1];
    if (numberInCell == 0) numberInCell = origBoardMemory[id][2];
    return numberInCell == 0;
  } else if (circleRadius === mini) {
    numberInCell = origBoardMemory[id][0];
    if (numberInCell == 0) numberInCell = origBoardMemory[id][1];
    if (numberInCell == 0) numberInCell = origBoardMemory[id][2];
    if (numberInCell == 0) numberInCell = origBoardMemory[id][3];
    return numberInCell == 0;
  }
}
